import { formatRef } from "../../data/incidents";

export default function ConfirmDialog({ incident, onCancel, onConfirm }) {
    if (!incident) {
        return null;
    }
            ...
            <div className="flex w-full max-w-[390px] flex-col gap-3 rounded-xl border border-ink/15 bg-white p-5 shadow-[0_18px_48px_rgba(22,32,43,.2)]">
                ...
                <div className="flex gap-2.5 pt-0.5">
                    ...
                    <button
                        type="button"
                        onClick={onConfirm}
                        className="flex-1 rounded-md border border-danger-600 bg-danger-600 px-4 py-2.5 text-[13px] font-semibold text-white hover:bg-danger-700"
                    >
                        Delete
                    </button>
                </div>
